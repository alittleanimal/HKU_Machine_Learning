...
class GameRules:
    ...
    def getPattern(self, board):
        ...
        pattern = [
            ('a', ["013578", "01568", "01567", "01456", "1357", "0268", "0178", "0138", "0135", "0134", "045", "027",
                   "024", "016", "17", "13", "08"]),
            ('b', ["02", "04", "05", "14", "013", "315", "0145", "0146", "0156", "0167", "0168", "0247",
                   "0457", "01357", "01358"]),
            ('c', [""]),
            ('cc', ["4"]),
            ('d', ["017", "018", "015"]),
        ...
        for p in pattern:
            for i in p[1]:
                if self.isBoardDeepEq(self.generatePattern(i), board):
                    return p[0]

        return "Error"

    def generatePattern(self, id):
    ...
    def transformPosition(self, board, xy):
        """
    ...
    def isBoardEqual(self, board1, board2):
        """
    ...
    def isBoardDeepEq(self, board1, board2):
        """
          Check if 2 boards are completely the same after rotation and reflection
        """
        for i in range(4):
            if self.isBoardEqual(board1, board2) or self.isBoardEqual(self.transformPosition(board1, 'x'), board2):
                return True
            board1 = self.transformPosition(board1, 'r')
        return False
...
class TicTacToeAgent:
    ...
    def getAction(self, gameState, gameRules):
        ...
        tmp = self.alphaBetaPruning(gameState, gameRules, 0, -10000, 10000, 0)
        return tmp[1]

    def filterActions(self, gameState, gameRules):
        legalActions = gameState.getLegalActions(gameRules)
        actionList = []
        existPattern = []
        for action in legalActions:
            if gameRules.fullPattern(gameState.generateSuccessor(action).boards) not in existPattern:
                existPattern.append(gameRules.fullPattern(gameState.generateSuccessor(action).boards))
                actionList.append(action)
        return actionList

    def alphaBetaPruning(self, gameState, gameRules, agentIndex, alpha, beta, depth):
        depth += 1
        ...
        if depth >= 4:
            ...
            # c2, a, b2, bc are supposed good choice
            if pattern in ['cc', 'a', 'bb', 'bc']:
                return [99]
            else:
                return [-99]
        ...
        valueMax = (-10000, '/')
        valueMin = (10000, '/')
        actions = self.filterActions(gameState, gameRules)
        # print(agentIndex,depth, actions)
        for a in actions:
            nextState = self.alphaBetaPruning(gameState.generateSuccessor(a), gameRules, agentIndex + 1,
                                              alpha, beta, depth)

            if agentIndex == 0:
                if valueMax[0] < nextState[0]:
                    valueMax = (nextState[0], a)
                if valueMax[0] > beta:
                    return valueMax
                alpha = max(alpha, valueMax[0])
            else:
                if valueMin[0] > nextState[0]:
                    valueMin = (nextState[0], a)
                if valueMin[0] < alpha:
                    return valueMin
                beta = min(beta, valueMin[0])
