...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        qValue = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * sampleQ
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for key in featureList.keys():
            qValue = qValue + self.weights[key] * featureList[key]
        return qValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        featureList = self.featExtractor.getFeatures(state, action)
        for key in featureList.keys():
            self.weights[key] = self.weights[key] + self.alpha * difference * featureList[key]
